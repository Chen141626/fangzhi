...
import { UIVertexFormat } from "cc";
import { Director } from "cc";
import { JSB } from "cc/env";
import { TiledRenderData } from "cc";
import { RenderData } from "cc";
import { Size } from "cc";
import { TiledTile } from "cc";
import { Node } from "cc";
...
let flipTexture: (grid: __private._cocos_tiledmap_tiled_types__TiledGrid, gid: __private._cocos_tiledmap_tiled_types__MixedGID) => void;

let _accessor: __private._cocos_2d_renderer_static_vb_accessor__StaticVBAccessor = null!;
/**
 * simple 组装器
 * 可通过 `UI.simple` 获取该组装器。
 */
export const simple: IAssembler & Record<string, any> = {
    ensureAccessor() {
        if (!_accessor) {
            const device = director.root!.device;
            const batcher = director.root!.batcher2D;
            _accessor = new __private._cocos_2d_renderer_static_vb_accessor__StaticVBAccessor(device, UIVertexFormat.vfmtPosUvColor, this.vCount);
    ...
    createData(layer: TiledLayer) {
        ...
        return null as unknown as RenderData;
    },

    fillBuffers(layer: TiledLayer, renderer: __private._cocos_2d_renderer_i_batcher__IBatcher) {
...
b d
*/
function _flipTexture(inGrid: __private._cocos_tiledmap_tiled_types__TiledGrid, gid: __private._cocos_tiledmap_tiled_types__MixedGID): void {
    if (inGrid._rotated) {
        // 2:b   1:a
        // 4:d   3:c
        _uva.x = inGrid.r;
    ...
    // vice
    if (((gid as unknown as number) & 536870912) >>> 0) {
        tempVal = _uvb;
        _uvb = _uvc;
        _uvc = tempVal;
    }
    ...
    // flip x
    if (((gid as unknown as number) & 2147483648) >>> 0) {
        tempVal = _uva;
        _uva = _uvc;
        _uvc = tempVal;
    ...
    // flip y
    if (((gid as unknown as number) & 1073741824) >>> 0) {
        tempVal = _uva;
        _uva = _uvb;
        _uvb = tempVal;
...
b     c
   d
*/
function _flipDiamondTileTexture(inGrid: __private._cocos_tiledmap_tiled_types__TiledGrid, gid: __private._cocos_tiledmap_tiled_types__MixedGID): void {
    if (inGrid._rotated) {
        //       2:b
        // 4:d         1:a
        //       3:c
    ...
    // vice
    if (((gid as unknown as number) & __private._cocos_tiledmap_tiled_types__TileFlag.DIAGONAL) >>> 0) {
        tempVal = _uva;
        _uva = _uvb;
        _uvb = tempVal;
    ...
    // flip x
    if (((gid as unknown as number) & __private._cocos_tiledmap_tiled_types__TileFlag.HORIZONTAL) >>> 0) {
        tempVal = _uvb;
        _uvb = _uvc;
        _uvc = tempVal;
    }
    ...
    // flip y
    if (((gid as unknown as number) & __private._cocos_tiledmap_tiled_types__TileFlag.VERTICAL) >>> 0) {
        tempVal = _uva;
        _uva = _uvd;
        _uvd = tempVal;
    }
...
function packRenderData(): void {
    ...
    if (JSB) {
        tiledData.renderData = RenderData.add(UIVertexFormat.vfmtPosUvColor, _accessor);
        tiledData.renderData.drawInfoType = __private._cocos_2d_renderer_render_draw_info__RenderDrawInfoType.MIDDLEWARE;
    } else {
        tiledData.renderData = RenderData.add(UIVertexFormat.vfmtPosUvColor);
    }
...
async function traverseGrids(leftDown: { col: number, row: number }, rightTop: { col: number, row: number },
    ...
    let rowData: { maxCol: number, minCol: number } & __private.__types_globals__SafeRecord<number, { left: number, bottom: number; index: number }>;
    let col: number;
    let cols: number;
    let row: number;
    let rows: number;
    ...
    let tileSize: Size;
    let grid: __private._cocos_tiledmap_tiled_types__TiledGrid | undefined;
    let gid: __private._cocos_tiledmap_tiled_types__MixedGID = 0 as unknown as any;
    let left = 0;
    let bottom = 0;
    let right = 0;
    let top = 0; // x, y
    ...
    let tempData: {
        grid: __private._cocos_tiledmap_tiled_types__TiledGrid,
        colData: {
            left: number;
            bottom: number;
            index: number;
        },
        gid: __private._cocos_tiledmap_tiled_types__MixedGID;
    ...
    for (; (rows - row) * rowMoveDir >= 0; row += rowMoveDir) {
        ...
        for (; (cols - col) * colMoveDir >= 0; col += colMoveDir) {
            ...
            grid = texGrids.get((((gid as unknown as number) & 268435455) >>> 0) as unknown as number);
            if (!grid) continue;
            tempData.push({
                grid: grid,
                colData: colData,
    ...
    tempData.sort((a, b) => {
        return b.colData.bottom - a.colData.bottom
    })
    tempData.forEach(item => {
        let grid = item.grid;
        let colData = item.colData;
        let gid = item.gid;
