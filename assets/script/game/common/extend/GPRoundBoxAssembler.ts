...
interface IGPRoundBoxAssembler extends IAssembler {
    [key: string]: any;
    GetIndexBuffer(sprite: GPRoundBoxSprite): number[];
}

export const GPRoundBoxAssembler: IGPRoundBoxAssembler = {
    ...
    updateUVs(sprite: GPRoundBoxSprite) {
        if (!sprite.spriteFrame || !sprite.renderData) return;
        const renderData = sprite.renderData!;
